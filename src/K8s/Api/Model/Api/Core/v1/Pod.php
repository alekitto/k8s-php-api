...
class Pod
{
     ...
     *  - `"ClusterFirst"` indicates that the pod should use cluster DNS first unless hostNetwork is true,
     * if it is available, then fall back on the default (as determined by kubelet) DNS settings.
     ...
     *  - `"Default"` indicates that the pod should use the default (as determined by kubelet) DNS
     * settings.
     *  - `"None"` indicates that the pod should use empty DNS settings. DNS parameters such as nameservers
     * and search paths should be defined via DNSConfig.
     */
     ...
     * existing pod, use the pod's ephemeralcontainers subresource. This field is beta-level and available
     ...
     * spec.containers[*].securityContext.runAsGroup This is an alpha field and requires the IdentifyPodOS
     * feature
     */
     ...
     * info: https://git.k8s.io/enhancements/keps/sig-node/688-pod-overhead/README.md This field is
     ...
     *  - `"Pending"` means the pod has been accepted by the system, but one or more of the containers has
     * not been started. This includes time before being bound to a node, as well as time spent pulling
     ...
     *  - `"Succeeded"` means that all containers in the pod have voluntarily terminated with a container
     ...
     * to an error in communicating with the host of the pod. Deprecated: It isn't being set since 2015
