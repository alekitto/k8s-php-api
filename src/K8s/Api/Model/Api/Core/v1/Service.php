...
class Service
{
     ...
     * not rely on NodePorts. allocateLoadBalancerNodePorts may only be set for services with type
     * LoadBalancer and will be cleared if the type is changed to any other type. This field is alpha-level
     * and is only honored by servers that enable the ServiceLBNodePortControl feature.
     */
     ...
     * same value.
     *
     * Unless the "IPv6DualStack" feature gate is enabled, this field is limited to one value, which must
     * be the same as the clusterIP field.  If the feature gate is enabled, this field may hold a maximum
     * of two entries (dual-stack IPs, in either order).  These IPs must correspond to the values of the
     * ipFamilies field. Both clusterIPs and ipFamilies are governed by the ipFamilyPolicy field. More
     * info:
     * https://kubernetes.io/docs/concepts/services-networking/service/#virtual-ips-and-service-proxies
     */
    ...
    /**
     * IPFamilies is a list of IP families (e.g. IPv4, IPv6) assigned to this service, and is gated by the
     * "IPv6DualStack" feature gate.  This field is usually assigned automatically based on cluster
     * configuration and the ipFamilyPolicy field. If this field is specified manually, the requested
     * family is available in the cluster, and ipFamilyPolicy allows it, it will be used; otherwise
     * creation of the service will fail.  This field is conditionally mutable: it allows for adding or
     * removing a secondary IP family, but it does not allow changing the primary IP family of the Service.
     *  Valid values are "IPv4" and "IPv6".  This field only applies to Services of types ClusterIP,
     * NodePort, and LoadBalancer, and does apply to "headless" services.  This field will be wiped when
    ...
    /**
     * IPFamilyPolicy represents the dual-stack-ness requested or required by this Service, and is gated by
     * the "IPv6DualStack" feature gate.  If there is no value provided, then this field will be set to
     * SingleStack. Services can be "SingleStack" (a single IP family), "PreferDualStack" (two IP families
     * on dual-stack configured clusters or a single IP family on single-stack clusters), or
     * "RequireDualStack" (two IP families on dual-stack configured clusters, otherwise fail). The
     * ipFamilies and clusterIPs fields depend on the value of this field.  This field will be wiped when
     * updating a service to type ExternalName.
     */
     ...
     * the list. If this is not specified or empty, no topology constraints will be applied. This field is
     * alpha-level and is only honored by servers that enable the ServiceTopology feature. This field is
     * deprecated and will be removed in a future version.
     */
    public function getTopologyKeys(): ?array
    {
        return $this->spec->getTopologyKeys();
    }
    ...
    {
        $this->spec->setTopologyKeys($topologyKeys);
