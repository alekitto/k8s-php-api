 ...
 * @Kubernetes\Operation("deletecollection-all",path="/apis/batch/v1/namespaces/{namespace}/jobs",response="K8s\Api\Model\ApiMachinery\Apis\Meta\v1\Status")
 * @Kubernetes\Operation("patch",path="/apis/batch/v1/namespaces/{namespace}/jobs/{name}",response="static::class")
 * @Kubernetes\Operation("patch-status",path="/apis/batch/v1/namespaces/{namespace}/jobs/{name}/status",response="static::class")
...
class Job
{
    ...
    /**
     * FailedIndexes holds the failed indexes when backoffLimitPerIndex=true. The indexes are represented
     * in the text format analogous as for the `completedIndexes` field, ie. they are kept as decimal
     * integers separated by commas. The numbers are listed in increasing order. Three or more consecutive
     * numbers are compressed and represented by the first and last element of the series, separated by a
     * hyphen. For example, if the failed indexes are 1, 3, 4, 5 and 7, they are represented as "1,3-5,7".
     * This field is alpha-level. It can be used when the `JobBackoffLimitPerIndex` feature gate is enabled
     * (disabled by default).
     */
    public function getFailedIndexes(): ?string
    {
        return $this->status->getFailedIndexes();
    }
     ...
     */
    public function getTerminating(): ?int
    {
        return $this->status->getTerminating();
    }
