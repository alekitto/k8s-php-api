 ...
 *
 * This is a beta feature available on clusters that haven't disabled the EphemeralContainers feature
 * gate.
 */
class EphemeralContainer
{
     ...
     *  - `"Always"` means that kubelet always attempts to pull the latest image. Container will fail If
     * the pull fails.
     *  - `"IfNotPresent"` means that kubelet pulls if the image isn't present on disk. Container will fail
     * if the image isn't present and the pull fails.
     *  - `"Never"` means that kubelet never pulls an image, but only uses a local image. Container will
     ...
     *
     * Possible enum values:
     *  - `"FallbackToLogsOnError"` will read the most recent contents of the container logs for the
     * container status message when the container exits with an error and the terminationMessagePath has
     * no contents.
